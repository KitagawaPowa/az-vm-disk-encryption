...
# We are ommiting this from the original template because we are using the
# azurerm_linux_virtual_machine resource instead... which does not have a
...
# This output should be empty since we are not using a DNS label
...
# Confirm the use of the SSE/CMK Keys as Disk Encryption Sets
